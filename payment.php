				...
				<div class="wrapper">
					<div class="banner-div">
						...
						<form method="post">
							...
							<a href="javascript:void(0);" id="cardBtn" class="btn btn-sucess btn-sm">CARD PAYMENT</a>	
							<a href="javascript:void(0);" id="gcashBtn" class="btn btn-sucess btn-sm">GCASH PAYMENT</a>	
							<!-- CARD DETAILS -->
							<div id="cardDisplay" style="margin-top:40px;display:none">
									<div class="banner-section details card">
									<h3>Card Details 
										<ul>
						...
						<div class="banner-section details card" id="gcashDisplay" style="margin-top:40px;display:none">
							<form action="gcashpayment.php" method="POST">
									<h4>Payment Provider: <span style="color:red;">GCASH PAYMENT</span></h4>
									<h5>Total Amount to Pay : ₱<?php echo number_format($total,2,'.',','); ?></h5>
									<input type="hidden" value="<?php echo $_GET['purchaseid']?>" name="id" id="id">
									<input type="hidden" readonly value="<?php echo $total; ?>" id="totalpayment" name="totalpayment">
									<button  type="submit"  class="btn btn-sucess btn-sm">PAY NOW VIA GCASH</button>
							</form>
						</div>
					</div>
				</div>
	...
	<script>
		$(document).ready(function(){
			$('#cardBtn').on('click',function(e){
				...
				$('#gcashDisplay').css('display','none');
			})

			$('#gcashBtn').on('click',function(e){
				e.preventDefault();
				$('#cardDisplay').css('display','none');
				$('#gcashDisplay').css('display','block');
			})
		})
	</script>
</body>
